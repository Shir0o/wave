...
void main(List<String> args) {
  ...
  print('=' * 80);
  print('${"File".padRight(50)} | ${"Hits".padLeft(6)} | ${"Lines".padLeft(6)} | ${"Coverage".padLeft(8)}');
  print('-' * 80);
  ...
  for (final String file in sortedFiles) {
    ...
    final String displayFile = file.replaceAll(Directory.current.path + '/', '');
    print('${displayFile.padRight(50)} | ${lh.toString().padLeft(6)} | ${lf.toString().padLeft(6)} | ${pct.toStringAsFixed(2).padLeft(7)}%');
  }

  print('=' * 80);
  final double totalPct = totalLf > 0 ? (totalLh / totalLf) * 100 : 0.0;
  print('${"TOTAL".padRight(50)} | ${totalLh.toString().padLeft(6)} | ${totalLf.toString().padLeft(6)} | ${totalPct.toStringAsFixed(2).padLeft(7)}%');
  print('=' * 80);
  print('Target threshold: ${threshold.toStringAsFixed(2)}%');

  if (totalPct < threshold) {
    print('FAIL: Code coverage is below threshold! (Actual: ${totalPct.toStringAsFixed(2)}% < Target: ${threshold.toStringAsFixed(2)}%)');
    exit(1);
  } else {
    print('SUCCESS: Code coverage meets threshold! (Actual: ${totalPct.toStringAsFixed(2)}% >= Target: ${threshold.toStringAsFixed(2)}%)');
    exit(0);
  }
}
